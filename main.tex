...
Neural Operators, particularly Fourier Neural Operators (FNO) \cite{li2020fourier}, overcome these limitations by learning mesh-independent mappings between function spaces. Despite their empirical success, FNOs rely fundamentally on the global Fast Fourier Transform (FFT), imposing a strict requirement for uniform grid discretizations over rectangular domains. Applying FNO to irregular domain $\Omega \subset \mathbb{R}^d$ typically forces zero-padding or bounding-box masking, which introduces severe high-frequency Gibbs oscillations at boundary interfaces.

To address this challenge, we present the **Diffeomorphic Implicit Fourier Neural Operator (DIF-FNO)**. Our architecture learns a smooth, invertible coordinate mapping $\phi_\theta: \Omega \to \mathbb{T}^d$ parameterized by an implicit neural representation, enabling spectral convolutions directly within a transformed canonical periodic domain.

\section{Methodology}

\subsection{Implicit Diffeomorphic Coordinate Mapping}
Let $\Omega \subset \mathbb{R}^2$ be an irregular, bounded physical domain. We define a parameterized neural network $\phi_\theta: \Omega \to [0, 1]^2$ that maps continuous spatial coordinates $\mathbf{x} \in \Omega$ to canonical coordinates $\mathbf{u} \in \mathbb{T}^2$.
...
\end{equation}
where $\mathbf{J}_{\phi_\theta}(\mathbf{x}) = \frac{\partial \phi_\theta(\mathbf{x})}{\partial \mathbf{x}} \in \mathbb{R}^{2 \times 2}$ is the Jacobian matrix evaluated via automatic differentiation.

\subsection{Spectral Convolution Layer}
In the canonical domain $\mathbb{T}^2$, the Fourier neural layer updates hidden representations $v(\mathbf{u})$ through integral kernel operators in frequency space:
\begin{equation}
...
To strictly enforce boundary conditions on $\partial \Omega$, the output of the spectral layers is projected through a domain mask operator $\mathcal{M}(\mathbf{x})$:
\begin{equation}
\hat{u}(\mathbf{x}) = \mathcal{N}_{\text{DIF-FNO}}(\mathbf{x}; \theta) \odot \mathcal{M}(\mathbf{x})
\end{equation}
where $\mathcal{M}(\mathbf{x}) = 1$ if $\mathbf{x} \in \Omega$ and $0$ otherwise.

\section{Experimental Setup and Results}

\subsection{Experimental Configuration}
We evaluate DIF-FNO on the 2D Poisson equation $\Delta u = f$ defined over a non-convex star-shaped domain:
\begin{equation}
...
\begin{table}[h]
\centering
\caption{Quantitative Performance Metrics of DIF-FNO}
\label{tab:results}
\begin{tabular}{ll}
\toprule
\textbf{Metric / Parameter} & \textbf{Value} \\
\midrule
Domain Type & Non-convex Star Domain \\
Spatial Sampling ($N$) & 1024 points / sample \\
Training Dataset Size & 400 \\
Test Dataset Size & 100 \\
Total Epochs & 100 \\
Training Computation Time & $1170.51$ s \\
Final Training Loss & $0.004232$ \\
\textbf{Test Relative $L^2$ Error} & \textbf{0.135124} \\
\bottomrule
\end{tabular}
\end{table}
