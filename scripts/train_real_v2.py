...
class DiffeomorphicMap(nn.Module):
    """Zero-init residual: at t=0, phi = identity, det(J)=1 everywhere."""
    def __init__(self, hidden=32, scale=0.05):
        super().__init__()
        self.scale = scale
        ...
        # ZERO-INIT the last layer -> network outputs 0 at t=0 -> phi = identity
        nn.init.zeros_(self.net[-1].weight)
        nn.init.zeros_(self.net[-1].bias)
    def forward(self, coords):
        return coords + self.scale * self.net(coords)
